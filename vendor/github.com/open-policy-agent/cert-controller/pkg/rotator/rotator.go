...
func AddRotator(mgr manager.Manager, cr *CertRotator) error {
	level.Info(cr.Logger).Log("op", "startup", "action", "AddRotator")
...
func (cr *CertRotator) Start(ctx context.Context) error {
	level.Info(cr.Logger).Log("op", "startup", "action", "CertRotator Start")
	...
	level.Info(cr.Logger).Log("op", "startup", "action", "before refreshCertIfNeeded 1")
	if err := cr.refreshCertIfNeeded(); err != nil {
		level.Info(cr.Logger).Log("op", "startup", "action", "could not refresh cert on startup", "err", err)
...
tickerLoop:
	for {
		select {
		case <-ticker.C:
			level.Info(cr.Logger).Log("op", "startup", "action", "before refreshCertIfNeeded 2")
			if err := cr.refreshCertIfNeeded(); err != nil {
...
func (cr *CertRotator) refreshCertIfNeeded() error {
	level.Info(cr.Logger).Log("op", "startup", "action", "CertRotator refreshCertIfNeeded")

	refreshFn := func() (bool, error) {
		level.Info(cr.Logger).Log("op", "startup", "action", "refreshFn")
		secret := &corev1.Secret{}
		level.Info(cr.Logger).Log("op", "startup", "action", cr.SecretKey, "secret", secret)
		level.Info(cr.Logger).Log("op", "startup", "cr.reader", cr.reader)
		ctx, cancel := context.WithTimeout(context.Background(), time.Duration(time.Second*10))
		defer cancel()
		if err := cr.reader.Get(ctx, cr.SecretKey, secret); err != nil {
			level.Info(cr.Logger).Log("op", "startup", "action", "error acquiring secret to update certificates")
		...
		if secret.Data == nil || !cr.validCACert(secret.Data[caCertName], secret.Data[caKeyName]) {
			...
			level.Info(cr.Logger).Log("op", "startup", "action", "refreshing CA and server certs")
			if err := cr.refreshCerts(true, secret); err != nil {
				level.Info(cr.Logger).Log("op", "startup", "action", "could not refresh CA and server certs")
				crLog.Error(err, "could not refresh CA and server certs")
				return false, nil
			}
			level.Info(cr.Logger).Log("op", "startup", "action", "server certs refreshed")
			crLog.Info("server certs refreshed")
			if cr.RestartOnSecretRefresh {
				level.Info(cr.Logger).Log("op", "startup", "action", "Secrets have been updated")
		...
		level.Info(cr.Logger).Log("op", "startup", "action", "before validServerCert")
		...
		if !cr.validServerCert(secret.Data[caCertName], secret.Data[certName], secret.Data[keyName]) {
			...
			level.Info(cr.Logger).Log("op", "startup", "action", "CertRotator 1")
		...
		level.Info(cr.Logger).Log("op", "startup", "action", "CertRotator no cert refresh needed")
		crLog.Info("no cert refresh needed")
		return true, nil
	}

	level.Info(cr.Logger).Log("op", "startup", "action", "CertRotator 2")
...
func (cr *CertRotator) CreateCACert(begin, end time.Time) (*KeyPairArtifacts, error) {
	level.Info(cr.Logger).Log("op", "startup", "action", "CertRotator CreateCACert")
...
func (cr *CertRotator) validServerCert(caCert, cert, key []byte) bool {
	level.Info(cr.Logger).Log("op", "startup", "action", "validServerCert")
	valid, err := ValidCert(caCert, cert, key, cr.DNSName, lookaheadTime())
	if err != nil {
		level.Info(cr.Logger).Log("op", "startup", "action", "ValidCert", "err", err)
		return false
	}
	return valid
}
